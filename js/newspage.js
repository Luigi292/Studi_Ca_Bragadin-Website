// js/news.js - News page functionality (English version)
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function fetchNewsData() {
    try {
      ...
      showError('Unable to load news at the moment. Please try again later.');
      return [];
    }
  }
  ...
  function renderNewsItems(newsItems, sortBy = 'recent', page = 1) {
    ...
    if (totalPages > 1) {
      ...
      // Previous button
      if (page > 1) {
        paginationHTML += `<button class="pagination-button" data-page="${page - 1}">Previous</button>`;
      }
      ...
      // Next button
      if (page < totalPages) {
        paginationHTML += `<button class="pagination-button" data-page="${page + 1}">Next</button>`;
      }
  ...
  function renderPopularNews() {
    ...
    let popularNewsHTML = '<h3>Popular News</h3>';
    
    if (popularNews.length === 0) {
      popularNewsHTML += '<p>No popular news at the moment</p>';
    } else {
  ...
  function renderCategories() {
    ...
    let categoriesHTML = '<li><a href="#" class="active" data-category="all">All news</a></li>';
  ...
  function showNewsModal(newsItem) {
    ...
    modalBody.innerHTML = `
      ...
      <div class="modal-actions">
        <a href="${newsItem.pdf}" class="cta-button" download>Download Full PDF</a>
      </div>
    `;
  ...
  async function initializePage() {
    try {
      ...
      document.getElementById('popularNewsBox').innerHTML = '<h3>Popular News</h3><div class="loading-spinner"></div>';
      ...
      if (!newsData || !newsData.length || newsData.length === 0) {
        showNoNewsMessage();
        document.getElementById('popularNewsBox').innerHTML = '<h3>Popular News</h3><p>No news at the moment</p>';
        return;
      }
